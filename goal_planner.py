...
def send_prompt(prompt: str, api_config: APIConfig) -> str:
    if api_config.api_type == "ollama":
        ...
        return send_groq_prompt(prompt, api_config.model, api_config.api_key)
    else:
...
def ask_clarifying_questions(user_goal: str, api_config: APIConfig):
    ...
    Question 5: ...
    """
    response = send_prompt(prompt, api_config)
...
def select_api_and_model() -> APIConfig:
    ...
    if api_type == "ollama":
        model = Prompt.ask(
            "[bold cyan]Enter Ollama model name[/bold cyan]",
            default="llama3.1"
        )
        return APIConfig(api_type="ollama", model=model)
    elif api_type == "groq":
        model = Prompt.ask(
            "[bold cyan]Enter Groq model name[/bold cyan]",
            default="mixtral-8x7b-32768"
        )
        api_key = Prompt.ask("[bold cyan]Enter your Groq API key[/bold cyan]", password=True)
...
def main():
    ...
    questions = ask_clarifying_questions(user_goal, api_config)
    responses = get_user_responses(questions)
    clarifying_qas = list(zip(questions, responses))
    
    console.print("\n[bold cyan]Refining your goal...[/bold cyan]")
    refined_goal = refine_goal(user_goal, clarifying_qas, api_config)
    print_goal(refined_goal)
    
    console.print("\n[bold cyan]Analyzing your refined goal and creating a detailed plan...[/bold cyan]")
    task_plan_content = send_prompt(TASK_PLAN_PROMPT.format(user_goal=refined_goal), api_config)
    enhanced_task_plan_content = send_prompt(ENHANCE_PROMPT.format(user_goal=refined_goal, task_plan_content=task_plan_content), api_config)
